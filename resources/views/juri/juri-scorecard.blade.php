...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!formId) {
        console.error('Form ID tidak tersedia.');
        parameterBody.innerHTML = '<tr><td colspan="4" class="text-center">Form ID tidak tersedia.</td></tr>';
        return;
    }
    ...
    fetch(`/api/formpenilaian/${formId}/parameters`)
        ...
        .then(data => {
            ...
            data.parameters.forEach((param, index) => {
                ...
                row.innerHTML = `
                    <th class="text-center style="width: 20px;">${index + 1}</th>
                    <td>
                        <strong>${param.nama}</strong><br>
                        ${param.deskripsi}
                        <input type="hidden" name="parameter_id[]" value="${param.id}">
                    </td>
                    <td class="text-center style="width: 50px;">${param.bobot}%</td>
                    <td class="text-center style="width: 10px;">
                        <select name="score[]" data-bobot="${param.bobot}" class="form-control form-control-sm score-select" style="width: 60px; margin-left: auto; margin-right: auto;">
